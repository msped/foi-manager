...
class TestDocumentUpload:
    def test_foi_team_can_upload(self, auth_client, case):
        ...
        pdf = SimpleUploadedFile('response.pdf', b'%PDF fake', content_type='application/pdf')
        resp = auth_client.post(url, {'file': pdf, 'original_filename': 'response.pdf'}, format='multipart')
    ...
    def test_assignee_can_upload_to_assigned_case(self, assignee_client, assigned_case):
        url = reverse('documents:case-documents-list', kwargs={'case_pk': assigned_case.pk})
        pdf = SimpleUploadedFile('info.pdf', b'%PDF info', content_type='application/pdf')
        resp = assignee_client.post(url, {'file': pdf, 'original_filename': 'info.pdf'}, format='multipart')
        assert resp.status_code == 201
    ...
    def test_assignee_cannot_upload_to_unassigned_case(self, assignee_client, case):
        ...
        resp = assignee_client.post(url, {'file': pdf, 'original_filename': 'x.pdf'}, format='multipart')
        assert resp.status_code == 404

    def test_anonymous_cannot_upload(self, api_client, case):
        url = reverse('documents:case-documents-list', kwargs={'case_pk': case.pk})
        pdf = SimpleUploadedFile('x.pdf', b'%PDF x', content_type='application/pdf')
        resp = api_client.post(url, {'file': pdf, 'original_filename': 'x.pdf'}, format='multipart')
...
class TestDocumentList:
    def test_foi_team_can_list(self, auth_client, case, foi_team_user):
        ...
        CaseDocument.objects.create(case=case, file=pdf, original_filename='a.pdf', uploaded_by=foi_team_user)
        url = reverse('documents:case-documents-list', kwargs={'case_pk': case.pk})
        resp = auth_client.get(url)
        assert resp.status_code == 200
        assert resp.data['count'] == 1

    def test_assignee_can_list_assigned_case_docs(self, assignee_client, assigned_case, foi_team_user):
        pdf = SimpleUploadedFile('b.pdf', b'%PDF b', content_type='application/pdf')
        CaseDocument.objects.create(case=assigned_case, file=pdf, original_filename='b.pdf', uploaded_by=foi_team_user)
        url = reverse('documents:case-documents-list', kwargs={'case_pk': assigned_case.pk})
...
class TestDocumentVisibility:
    def test_foi_team_can_mark_public(self, auth_client, case, foi_team_user):
        ...
        doc = CaseDocument.objects.create(case=case, file=pdf, original_filename='resp.pdf', uploaded_by=foi_team_user)
        url = reverse('documents:case-documents-detail', kwargs={'case_pk': case.pk, 'pk': doc.pk})
