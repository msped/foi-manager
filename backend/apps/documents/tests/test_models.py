...
@pytest.fixture
def pdf_file():
    return SimpleUploadedFile('response.pdf', b'%PDF-1.4 fake content', content_type='application/pdf')
...
class TestCaseDocument:
    ...
    def test_ordering_newest_first(self, db, case, foi_team_user):
        ...
        CaseDocument.objects.create(case=case, file=f1, original_filename='a.pdf', uploaded_by=foi_team_user)
        CaseDocument.objects.create(case=case, file=f2, original_filename='b.pdf', uploaded_by=foi_team_user)
