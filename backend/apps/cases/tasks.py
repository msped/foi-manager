...
def task_send_consultation_notification(self, consultation_id: int):
    ...
    try:
        consultation = CaseConsultation.objects.select_related("assignee", "case").get(
            pk=consultation_id
        )
...
@app.task(bind=True, max_retries=3, default_retry_delay=60)
def task_send_case_assignment_notification(self, case_id: int, assignee_id: int):
    from django.contrib.auth import get_user_model

    from .email_utils import send_case_assignment_notification
    from .models import Case

    User = get_user_model()
    try:
        case = Case.objects.get(pk=case_id)
        assignee = User.objects.select_related("notification_preferences").get(
            pk=assignee_id
        )
        send_case_assignment_notification(case, assignee)
    except Exception as exc:
        raise self.retry(exc=exc)
...
def task_send_case_response(self, case_response_id: int):
    ...
    try:
        ...
        case_response = CaseResponse.objects.select_related("case").get(
            pk=case_response_id
        )
